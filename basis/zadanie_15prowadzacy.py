...
min_l_krokow_po_wylosow = abs(skarb_x - gracz_x) + abs(skarb_y - gracz_y)
liczba_krokow = 0
DEBUG = False
...
while True:
    ...
    if DEBUG:
        ...
        print("Minimalna liczba krokow:", min_l_krokow_po_wylosow)
    ...
    if gracz_x == skarb_x and gracz_y == skarb_y:
        print("Wygrałeś!")
        print(f"Minimalna liczba kroków wynosiła: {min_l_krokow_po_wylosow}")
    ...
    if liczba_krokow >= min_l_krokow_po_wylosow * 2:
        skarb_x = randint(1, 10)
        skarb_y = randint(1, 10)
        min_l_krokow_przed_ruchem = abs(skarb_x - gracz_x) + abs(skarb_y - gracz_y)
        liczba_krokow = 0
